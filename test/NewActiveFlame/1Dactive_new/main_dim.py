...
D = ActiveFlame(mesh, subdomains,
                    w, rho, params_dim.Q_tot, params_dim.U_bulk, ftf,x_r, 
                    degree=degree)
...
# print((local_size[0], global_size), (local_size, global_size))
#print(local_size)
# print("Nonzero entries:", np.count_nonzero(D.matrix[range(local_size),range(local_size)]))
# total_nonzero = comm.allreduce(np.count_nonzero(D.matrix[range(local_size),range(local_size)]), op=MPI.SUM)
# print("Nonzero entries:",total_nonzero)
