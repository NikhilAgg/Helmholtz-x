...
$$ D_{jk} = \underbrace{\overbrace{(\gamma-1) \frac{ q_{tot}  }{ U_{bulk}}}^{\text{coefficient}} \int_{\Omega} \phi_i n h(\textbf{x}) e^{i \omega \tau(\textbf{x})} d\textbf{x} }_{\text{left vector}}  \underbrace{\int_{\Omega} \frac{w(\chi)}{\rho_0 (\chi)}  \nabla{\phi_j} \cdot \textbf{n}_{ref} d\chi }_{\text{right vector}} $$
