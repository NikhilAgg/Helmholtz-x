...
class ActiveFlame:
    ...
    def _assemble_right_vector(self, point):
        ...
        if tdim==1:
            ...
            print("2 Dimensional Geometry")
            n = as_vector([1,0])
        else:
            n = as_vector([0,0,1])

        right_vector = assemble_vector(form(inner(n,grad(self.v)*self.w/self.rho)*ufl.dx))
        tol = 1e-16
        right_vector.array.real[abs(right_vector.array.real) < tol] = 0.0
        right_vector.array.imag[abs(right_vector.array.imag) < tol] = 0.0

        # print(right_vector.array)
        indices2 = np.array(np.flatnonzero(right_vector.array),dtype=np.int32)
        dofmaps = self.V.dofmap
        global_indices = dofmaps.index_map.local_to_global(indices2)
        right_vector = list(zip(global_indices, right_vector[indices2]))
        # print("B ",right_vector)
        return right_vector
