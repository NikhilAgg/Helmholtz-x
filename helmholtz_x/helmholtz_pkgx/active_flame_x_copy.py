...
class ActiveFlame:
    ...
    def _assemble_right_vector(self, point):
        ...
        bb = BoundingBoxTree(domain, tdim)
        bbox_collisions = compute_collisions(bb, point)
        print("Collisions: ", bbox_collisions)
        cells = compute_colliding_cells(domain, bbox_collisions, point)
        print("Colliding cells: ", cells, type(cells))
        # cell = cells[0]
        print("tdim: ", tdim)
        ...
        if len(cells) > 0:
            ...
            geom_dofs = self.mesh.geometry.dofmap.links(cell)
            x_ref = cmap.pull_back([point[:gdim]], self.mesh.geometry.x[geom_dofs])
            print("x_ref")
            expr = inner(grad(TestFunction(self.V)), n)
            
            E = dolfinx.fem.Expression(expr, x_ref)
            print("Expression")
            grad_u_at_x_ref = E.eval([cell])
            print("grad_u_at_x_ref")
            for i in range(len(geom_dofs)):
                B.append([geom_dofs[i], grad_u_at_x_ref[0][i]])
                
        else:
            print(MPI.COMM_WORLD.rank, "Ghost", cells) 
        root = 0 #it was -1
        if len(B) > 0:
